...
puts number_of_usernames_with_number
...
def majuscule 
  count = 0
  @journalists.each do |journalist|
    count += 1 if ('A'..'Z').include?(journalist[1])
  end
...
def number_capitals
  count = 0
  @journalists.each do |journalist|
    (0...journalist.length).each do |i|
      if ('A'..'Z').cover?(journalist[i])
        count += 1
        break
      end
    end
...
puts "> Trie de a liste des emails journalistes par ordre alphabétique"

def alphajournalists
